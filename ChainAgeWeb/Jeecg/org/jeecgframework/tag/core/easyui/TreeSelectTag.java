...
public class TreeSelectTag extends TagSupport {
    ...
    private void showTreeFunction(StringBuffer resultSb) {
        ...
        if ("ace".equals(formStyle)) {
            resultSb.append(
                    " $(\"#show" + StringUtil.firstUpperCase(field) + "TreeContent\").css({top:(deptObj[0].offsetTop + deptObj.outerHeight()) + \"px\"}).slideDown(\"fast\");  ");
            resultSb.append("$('#show" + StringUtil.firstUpperCase(field) + "Tree').css({width:deptObj.outerWidth() - 2 + \"px\"});  ");
        } else {
            resultSb.append(" $(\"#show" + StringUtil.firstUpperCase(field)
    ...
    private void getCheckFunction(StringBuffer resultSb) {
        ...
        /*
         * if(StringUtil.isNotEmpty(id)){ resultSb.append("var idVal = $(\"#"+id+"\").val();");
         * resultSb.append(" var textVal = $(\"#"+id+"Text\").val();"); }else{
         * resultSb.append("var idVal = $(\"#"+field+"\").val();");
         * resultSb.append(" var textVal = $(\"#"+field+"Text\").val();"); }
         * resultSb.append(" if(treeNode.checked){"); resultSb.append("if(idVal != null && idVal != ''){");
         * if(StringUtil.isNotEmpty(id)){ resultSb.append("$(\"#"+id+"\").val(idVal + ',' +treeNode.id); ");
         * }else{ resultSb.append("$(\"#"+field+"\").val(idVal + ',' +treeNode.id); "); }
         * resultSb.append("}else{"); if (StringUtil.isNotEmpty(id)) {
         * resultSb.append(" $(\"#"+id+"\").val(treeNode.id); "); }else{
         * resultSb.append(" $(\"#"+field+"\").val(treeNode.id); "); } resultSb.append("}");
         * resultSb.append("if(textVal != null && textVal != ''){"); if(StringUtil.isNotEmpty(id)){
         * resultSb.append("$(\"#"+id+"Text\").val(textVal + ',' + treeNode.text); "); }else{
         * resultSb.append("$(\"#"+field+"Text\").val(textVal + ',' + treeNode.text); "); }
         * resultSb.append("}else{"); if(StringUtil.isNotEmpty(id)){
         * resultSb.append("$(\"#"+id+"Text\").val(treeNode.text); "); }else{
         * resultSb.append("$(\"#"+field+"Text\").val(treeNode.text); "); } resultSb.append("}");
         * resultSb.append("}else{"); resultSb.append("idVal = idVal.replace(treeNode.id,'');");
         * resultSb.append("if(idVal.indexOf(',') == 0){"); resultSb.append("idVal = idVal.substring(1);");
         ...
         * resultSb.append("idVal = idVal.substring(0,idVal.length - 1);"); resultSb.append("}");
         ...
         * resultSb.append("textVal = textVal.substring(0,textVal.length - 1);"); resultSb.append("}");
         * if(StringUtil.isNotEmpty(id)){ resultSb.append("$(\"#"+id+"Text\").val(textVal);");
         * resultSb.append(" $(\"#"+id+"\").val(idVal);"); }else{
         * resultSb.append("$(\"#"+field+"Text\").val(textVal);");
         * resultSb.append(" $(\"#"+field+"\").val(idVal);"); } resultSb.append("}");
         */
