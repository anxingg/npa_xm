...
public class TabsTag extends JeecgTag {
    ...
    public StringBuffer end() {
        ...
        if (iframe) {
            ...
            if (tabList.size() > 0) {
                for (Tab tab : tabList) {
                    sb.append(
                            "add" + id + "(\'" + tab.getTitle() + "\',\'" + tab.getHref() + "\',\'" + tab.getId() + "\',\'" + tab.getIcon() + "\',\'" + tab.isClosable() + "\');");
                }
            }
            ...
            sb.append("var s = \'<iframe id=\"+id+\" scrolling=\"no\" frameborder=\"0\"  src=\"about:blank\" width=\"" + oConvertUtils.getString(width, "100%") + "\" height=\""
        ...
        if (tabs) {
            ...
            sb.append("<div id=\"" + id + "\" tabPosition=\"" + tabPosition + "\" border=flase style=\"margin:0px;padding:0px;overflow-x:hidden;width:"
                    + oConvertUtils.getString(width, "auto") + ";\" class=\"easyui-tabs\" fit=\"" + fit + "\">");
            if (!iframe) {
                for (Tab tab : tabList) {
                    if (tab.getHref() != null) {
                        sb.append("<div title=\"" + tab.getTitle() + "\" " + (tab.getIcon() != null ? ("iconCls=\"" + tab.getIcon() + "\" ") : "") + " href=\"" + tab.getHref()
                                + "\" style=\"margin:0px;padding:0px;overflow-x:hidden;overflow-y:auto;width=auto;\"></div>");
                    } else {
                        sb.append("<div " + (tab.getIcon() != null ? ("iconCls=\"" + tab.getIcon() + "\" ") : "") + " title=\"" + tab.getTitle()
                                + "\"  style=\"margin:0px;padding:0px;overflow-x:hidden;overflow-y:auto;width=auto;\">");

                        sb.append("<iframe id=\"" + tab.getId() + "\" scrolling=\"no\" frameborder=\"0\"  src=\"" + tab.getIframe() + "\" width=\""
                                + oConvertUtils.getString(tab.getWidth(), "100%") + "\" height=\"" + oConvertUtils.getString(tab.getHeigth(), "99.5%") + "\"></iframe>");
    ...
    public String toString() {
        ...
        builder.append("TabsTag [id=").append(id).append(", width=").append(width).append(", heigth=").append(heigth).append(", plain=").append(plain).append(", fit=").append(fit)
                .append(", border=").append(border).append(", scrollIncrement=").append(scrollIncrement).append(", scrollDuration=").append(scrollDuration).append(", tools=")
                .append(tools).append(", tabs=").append(tabs).append(", iframe=").append(iframe).append(", tabPosition=").append(tabPosition);
