...
public class FormUtil {
    ...
    private static String GetMacros(JSONObject item, Map<String, Object> formData, String action) {
        ...
        if ("view".equals(action))
            return value.replace("${userId}", ClientManager.getInstance().getClient().getUser().getId()).replace("${userName}",
                    ClientManager.getInstance().getClient().getUser().getUserName());
        if (value != null) {
    ...
    private static String GetListctrl(JSONObject item, Map<String, Object> formData, String action) {
        ...
        String temp = "<table id=\"" + name + "_table\" bindTable=\"true\" cellspacing=\"0\" class=\"table table-bordered table-condensed\" style=\"" + style
