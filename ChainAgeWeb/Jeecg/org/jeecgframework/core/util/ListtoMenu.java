...
public class ListtoMenu {
    ...
    public static String getBootMenu(List<TSFunction> pFunctions, List<TSFunction> functions) {
        ...
        for (TSFunction pFunction : pFunctions) {
             ...
             * menuString.append("</li>"); } if (submenusize > 0) { menuString.append("<ul class=\"sub\">"); }
             * for (TSFunction function : functions) {
             * 
             * if (function.getTSFunction().getId().equals(pFunction.getId())) { menuString
    ...
    public static String getEasyuiMenu(List<TSFunction> pFunctions, List<TSFunction> functions) {
        ...
        for (TSFunction pFunction : pFunctions) {
            ...
            /*
             * int submenusize = pFunction.getSubFunctionSize(); if (submenusize == 0) {
             * menuString.append("</div>"); } if (submenusize > 0) { menuString.append("<ul>"); } for
             * (TSFunction function : functions) {
             * 
             * if (function.getTSFunction().getId().equals(pFunction.getId())) { String icon = "folder"; if
             * (function.getTSIcon() != null) { icon =
             ...
             * "</span></a></div></li>"); } } if (submenusize > 0) { menuString.append("</ul></div>"); }
             */
            ...
            for (TSFunction function : functions) {
                if (function.getTSFunction().getId().equals(pFunction.getId())) {
                    ...
                    menuString.append("<li><div onclick=\"addTab(\'" + getMutiLang(function.getFunctionName()) + "\',\'" + funUrl + "\',\'" + icon + "\')\"  title=\""
                            + getMutiLang(function.getFunctionName()) + "\" url=\"" + function.getFunctionUrl() + "\" iconCls=\"" + icon + "\"> <a class=\""
                            + getMutiLang(function.getFunctionName()) + "\" href=\"#\" > <span class=\"icon " + icon + "\" >&nbsp;</span> <span class=\"nav\" >"
        ...
        for (TSFunction function : list) {
            if (function.getTSFunction().getId().equals(parent.getId())) {
                if (!function.hasSubFunction(map)) {
                    ...
                    menuString.append("<li state=\"closed\" iconCls=\"" + ResourceUtil.allTSIcons.get(function.getTSIcon().getId()).getIconClas() + "\" ><span>"
    ...
    public static String getBootstrapMenu(Map<Integer, List<TSFunction>> map) {
        ...
        for (TSFunction pFunction : pFunctions) {
            ...
            menuString.append("			<span class=\"bootstrap-icon\" style=\"background-image: url('" + ResourceUtil.allTSIcons.get(pFunction.getTSIcon().getId()).getIconPath()
                    + "')\"></span> " + pFunction.getFunctionName() + " ");
            if (hasSub) {
    ...
    /**
     * @Title: getBootStrapChild @Description: 递归获取bootstrap的子菜单 @param parent @param level @param
     * map @return String @throws
     */
    private static String getBootStrapChild(TSFunction parent, int level, Map<Integer, List<TSFunction>> map) {
        ...
        for (TSFunction function : list) {
            if (function.getTSFunction().getId().equals(parent.getId())) {
                ...
                menuString.append("		<li onclick=\"showContent(\'" + getMutiLang(function.getFunctionName()) + "\',\'" + menu_url + "\')\"  title=\""
    ...
    /**
     * @Title: getWebosChild @Description: 递归获取Webos的子菜单 @param parent @param level @param map @return
     * String @throws
     */
    private static String getWebosChild(TSFunction parent, int level, Map<Integer, List<TSFunction>> map) {
        ...
        for (TSFunction function : list) {
            if (function.getTSFunction().getId().equals(parent.getId())) {
                boolean hasSub = function.hasSubFunction(map);
                menuString.append("\"" + function.getId() + "\":");
                menuString.append("{\"id\":\"" + function.getId() + "\",\"name\":\"" + getMutiLang(function.getFunctionName()) + "\",\"path\":\""
                        + function.getTSIcon().getIconPath() + "\",\"url\":\"" + function.getFunctionUrl() + "\",\"level\":\"" + function.getFunctionLevel() + "\"}");
    ...
    /**
     * @Title: getMutiLang @Description: 转换菜单多语言 @param functionName @return String @throws
     */
        ...
        for (TSFunction function : list) {
            if (function.getTSFunction().getId().equals(parent.getId())) {
                if (!function.hasSubFunction(map)) {
                    ...
                    menuString.append("<li><a href=\"#\" ><i class=\"menu-icon fa fa-eye pink\" iconCls=\"" + icon + "\" ></i>" + getMutiLang(function.getFunctionName())
                            + "<b class=\"arrow\"></b>");
            ...
            if (function.getTSFunction().getId().equals(parent.getId())) {
                if (!function.hasSubFunction(map)) {
                    ...
                    if (function.getFunctionIconStyle() != null && !function.getFunctionIconStyle().trim().equals("")) {
                        ...
                        menuString
                                .append("<a href=\"#\" class=\"dropdown-toggle\" ><i class=\"" + SysACEIconEnum.toEnum(function.getTSIcon().getIconClas()).getThemes() + "\"></i>");
                    }
                    menuString.append("<span class=\"menu-text\">");
                    menuString.append(getMutiLang(function.getFunctionName()));
                    menuString.append("</span>");
