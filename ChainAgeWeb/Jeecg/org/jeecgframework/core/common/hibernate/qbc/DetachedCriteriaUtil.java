...
// Set<String> aliases = (Set<String>)
// ContextHolderUtils.getRequest().getAttribute(ALIAS_KEY_IN_REQUEST);
// if(aliases==null){
...
// Boolean hasJoin = (Boolean)
// ContextHolderUtils.getRequest().getAttribute(HAS_JOIN_TABLE_KEY_IN_REQUEST);
...
// if(hasJoniTable || forJoinTable ||
// getHasJoinTatleFromRequest()){//这个一定要放在tempProjectionList的前面，因为distinct要在最前面
...
// * @param currentStep
// */
// private static void createAlias(DetachedCriteria criteria, String rootAlais, Set<String>
// aliases,String[] columns,int currentStep){
...
// converArrayToAlias(columns, currentStep)).setFetchMode(columns[currentStep], FetchMode.JOIN);
// }else{
// criteria.createAlias(rootAlais + POINT +columns[currentStep], converArrayToAlias(columns,
// currentStep)).setFetchMode(columns[currentStep], FetchMode.JOIN);
// }
