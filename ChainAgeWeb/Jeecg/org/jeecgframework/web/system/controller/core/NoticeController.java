...
public class NoticeController extends BaseController {
    ...
    public AjaxJson getNoticeList(Integer isRead, HttpServletRequest req) {
        ...
        try {
            ...
            String sql = "SELECT notice.*,noticeRead.is_read as is_read FROM t_s_notice notice " + "LEFT JOIN t_s_notice_read_user noticeRead ON  notice.id = noticeRead.notice_id "
            ...
            // 获取通知公告总数
            String sql2 = "SELECT count(notice.id) FROM t_s_notice notice " + "LEFT JOIN t_s_notice_read_user noticeRead ON  notice.id = noticeRead.notice_id "
    ...
    public void datagrid(TSNotice notice, HttpServletRequest request, HttpServletResponse response, DataGrid dataGrid) {
        ...
        // //查询条件组装器
        // org.jeecgframework.core.extend.hqlsearch.HqlGenerateUtil.installHql(cq, notice,
        // request.getParameterMap());
        // this.noticeService.getDataGridReturn(cq, true);

        TSUser user = ResourceUtil.getSessionUser();
        String sql = "SELECT notice.*,noticeRead.is_read as is_read FROM t_s_notice notice " + " LEFT JOIN t_s_notice_read_user noticeRead ON  notice.id = noticeRead.notice_id "
                + " WHERE noticeRead.del_flag = 0 and noticeRead.user_id = ? " + " ORDER BY noticeRead.is_read asc,noticeRead.create_time DESC ";
