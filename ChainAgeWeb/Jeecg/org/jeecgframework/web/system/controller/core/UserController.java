...
public class UserController extends BaseController {
    ...
    public AjaxJson del(TSUser user, HttpServletRequest req) {
        ...
        if (!user.getStatus().equals(Globals.User_ADMIN)) {
             ...
             * 
             * systemService.executeSql("delete from t_s_user_org where user_id=?", user.getId()); // 删除 用户-机构
             * 数据
             * 
    ...
    public AjaxJson trueDel(TSUser user, HttpServletRequest req) {
        ...
        /*
         * List<TSRoleUser> roleUser = systemService.findByProperty(TSRoleUser.class, "TSUser.id",
         * user.getId()); if (!user.getStatus().equals(Globals.User_ADMIN)) { if (roleUser.size()>0) { //
         * 删除用户时先删除用户和角色关系表 delRoleUser(user);
         * systemService.executeSql("delete from t_s_user_org where user_id=?", user.getId()); // 删除 用户-机构
         * 数据 userService.delete(user); message = "用户：" + user.getUserName() + "删除成功";
         * systemService.addLog(message, Globals.Log_Type_DEL, Globals.Log_Leavel_INFO); } else {
         * userService.delete(user); message = "用户：" + user.getUserName() + "删除成功"; } } else { message =
         * "超级管理员不可删除"; }
         */
    ...
    /*
     * public void delRoleUser(TSUser user) { // 同步删除用户角色关联表 List<TSRoleUser> roleUserList =
     * systemService.findByProperty(TSRoleUser.class, "TSUser.id", user.getId()); if
     * (roleUserList.size() >= 1) { for (TSRoleUser tRoleUser : roleUserList) {
    ...
    public ModelAndView addorupdate(TSUser user, HttpServletRequest req) {
         ...
         * oConvertUtils.getString(req.getParameter("departid")); if(!StringUtil.isEmpty(departid)){
         * departList.add((TSDepart)systemService.getEntity(TSDepart.class,departid)); }else {
         * departList.addAll((List)systemService.getList(TSDepart.class)); } req.setAttribute("departList",
         * departList);
         */
    ...
    /**
     * @Title: saveStyle @Description: 修改首页样式 @param request @return AjaxJson @throws
     */
