            ...
            for (byte[] data : getData) {
                ...
                if (data.length >= 8) {
                    ...
                    case MsgCommand.CMPP_CONNECT_RESP:
                        ...
                        logger.info("快消平台" + new SimpleDateFormat("yyyy-MM-dd HH:mm:ss").format(new Date()) + "链接短信网关,状态:" + connectResp.getStatusStr() + " 序列号："
                    ...
                    case MsgCommand.CMPP_SUBMIT_RESP:
                        ...
                        logger.info("快消平台" + new SimpleDateFormat("yyyy-MM-dd HH:mm:ss").format(new Date()) + "向用户下发短信，状态码:" + submitResp.getResult() + " 序列号："
                    ...
                    case MsgCommand.CMPP_DELIVER:
                        ...
                        if (msgDeliver.getResult() == 0) {
                            logger.info("CMPP_DELIVER 序列号：" + head.getSequenceId() + "，是否消息回复"
                                    + (msgDeliver.getRegisteredDelivery() == 0 ? "不是,消息内容：" + msgDeliver.getMsgContent() : "是，目的手机号：" + msgDeliver.getDestTerminalId()));
                        } else {
