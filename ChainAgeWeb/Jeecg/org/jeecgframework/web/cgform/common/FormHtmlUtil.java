...
public class FormHtmlUtil {
    ...
    private static String getRadioFormHtml(CgFormFieldEntity cgFormFieldEntity) {
        ...
        if (StringUtil.isEmpty(cgFormFieldEntity.getDictField())) {
            ...
            // html.append("<#if
            // dictdata.typecode=='\\${").append(cgFormFieldEntity.getFieldName()).append("?if_exists?html}'>");
            html.append("<#if dictdata.typecode==\"\\${data\\['" + cgFormFieldEntity.getTable().getTableName() + "'\\]\\['").append(cgFormFieldEntity.getFieldName())
                    .append("'\\]?if_exists?html}\">");
    ...
    private static String getCheckboxFormHtml(CgFormFieldEntity cgFormFieldEntity) {
        if (StringUtil.isEmpty(cgFormFieldEntity.getDictField())) {
            ...
            // html.append("<#assign
            // checkboxstr>\\${").append(cgFormFieldEntity.getFieldName()).append("?if_exists?html}</#assign>");
            html.append("<#assign checkboxstr>\\${data\\['" + cgFormFieldEntity.getTable().getTableName() + "'\\]\\['").append(cgFormFieldEntity.getFieldName())
                    .append("'\\]?if_exists?html}</#assign>");
    ...
    private static String getFileFormHtml(CgFormFieldEntity cgFormFieldEntity) {
        ...
        // html.append("<link rel=\"stylesheet\" href=\"plug-in/uploadify/css/uploadify.css\"
        // type=\"text/css\"></link>");
        ...
        html.append("\\$('#").append(cgFormFieldEntity.getFieldName()).append("').uploadify(\"settings\", \"formData\", {'cgFormId':cgFormId,'cgFormName':'")
                .append(cgFormFieldEntity.getTable().getTableName()).append("','cgFormField':'").append(cgFormFieldEntity.getFieldName()).append("'});} ,");
        ...
        html.append(
                "onUploadSuccess : function(file, data, response) {var d=\\$.parseJSON(data);if(d.success){var win = frameElement.api.opener;serverMsg = d.msg;}},onFallback : function(){tip(\"您未安装FLASH控件，无法上传图片！请安装FLASH控件后再试\")},onSelectError : function(file, errorCode, errorMsg){switch(errorCode) {case -100:tip(\"上传的文件数量已经超出系统限制的\"+\\$('#")
                .append(cgFormFieldEntity.getFieldName())
                .append("').uploadify('settings','queueSizeLimit')+\"个文件！\");break;case -110:tip(\"文件 [\"+file.name+\"] 大小超出系统限制的\"+\\$('#")
                .append(cgFormFieldEntity.getFieldName())
                .append("').uploadify('settings','fileSizeLimit')+\"大小！\");break;case -120:tip(\"文件 [\"+file.name+\"] 大小异常！\");break;case -130:tip(\"文件 [\"+file.name+\"] 类型不正确！\");break;}},");
        html.append("onUploadProgress : function(file, bytesUploaded, bytesTotal, totalBytesUploaded, totalBytesTotal) { }});});");
        html.append("</script><span id=\"file_uploadspan\"><input type=\"file\" name=\"").append(cgFormFieldEntity.getFieldName()).append("\" id=\"")
                .append(cgFormFieldEntity.getFieldName()).append("\" /></span>");
        ...
        // html.append("if(data.obj != null && data.obj !=
        // 'undefined'){\\$(\"input[name='id']\").val(data.obj.id);}}");
        // html.append("if(\\$(\".uploadify-queue-item\").length > 0){upload();}else{if
        // (neibuClickFlag){alert(data.msg); neibuClickFlag = false;}else {var win =
        // frameElement.api.opener; win.reloadTable(); win.tip(data.msg); frameElement.api.close();}}}");
        ...
        // html.append("var neibuClickFlag = false;function neibuClick() {neibuClickFlag =
        // true;\\$('#btn_sub').trigger('click');}");
        html.append("</script>");
    ...
    private static String getPopupFormHtml(CgFormFieldEntity cgFormFieldEntity) {
        ...
        html.append(
                "onclick=\"popupClick(this,'" + cgFormFieldEntity.getDictText() + "','" + cgFormFieldEntity.getDictField() + "','" + cgFormFieldEntity.getDictTable() + "');\" ");
