...
public class CgFormIndexServiceImpl extends CommonServiceImpl implements CgFormIndexServiceI {
    ...
    public boolean updateIndexes(CgFormHeadEntity cgFormHead) {
        ...
        List<CgFormIndexEntity> oldindexes =
                this.getSession().createSQLQuery("select * from cgform_index where table_id = '" + cgFormHead.getId() + "'").addEntity(CgFormIndexEntity.class).list();
        if (oldindexes.size() != 0 && indexes != null) {
            if (oldindexes.size() != indexes.size()) {
                ...
                for (int i = 0; i < indexes.size(); i++) {
                    ...
                    if (oldindex.getIndexField().equals(newindex.getIndexField()) && oldindex.getIndexName().equals(newindex.getIndexName())
                            && oldindex.getIndexType().equals(newindex.getIndexType())) {
         ...
         * createSQLQuery("select * from cgform_index where table_id = '" + cgFormHead.getId() +
         * "'").addEntity(CgFormIndexEntity.class).list();
         */
    ...
    public void createIndexes(CgFormHeadEntity cgFormHead) {
        CgFormHeadEntity formhead = this.getEntity(CgFormHeadEntity.class, cgFormHead.getId());
        List<CgFormIndexEntity> indexes =
                this.getSession().createSQLQuery("select * from cgform_index where table_id = '" + cgFormHead.getId() + "'").addEntity(CgFormIndexEntity.class).list();
        if (indexes.size() != 0) {
            for (CgFormIndexEntity cgform : indexes) {
